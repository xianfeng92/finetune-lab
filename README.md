...
`finetune-lab` 会把训练结果放回 held-out probe 和 case-level diff 里看：模型到底有没有选对工具、填对参数、输出合法 JSON、做出正确行为；涉及端侧部署时，还要复测同一 LoRA 在目标 runtime 上是否保留安全 contract。
...
4. **case diff 可视化** —— 同屏看 input / expected / actual，定位模型到底哪里保留了行为、哪里出现退化
...
- 🧯 **runtime reality check** — 同一 LoRA 在 MLX adapter 和 GGUF fused runtime 上分别复测，避免把训练侧结果误写成部署侧结论
...
如果这个项目帮你把微调看清楚了，欢迎 star；如果哪里仍然像黑盒，也欢迎直接提 issue。

- **学习卡点**：用 `Learning question` 模板告诉我哪一步还没讲透
- **新 recipe**：用 `Recipe request` 模板提一个你想复现的微调问题
- **工程改进**：先看 [CONTRIBUTING.md](CONTRIBUTING.md)，尽量附上命令和 artifact 路径
...
| 学习路径 | 你会看懂什么 | 当前入口 |
|---|---|---|
| [`loss-is-lying`](docs/recipes/loss-is-lying.md) | 为什么 loss 降了，模型仍然可能没学会 | [Training Runs](https://xianfeng92.github.io/finetune-lab/#/runs) + [Probe Compare](https://xianfeng92.github.io/finetune-lab/#/compare) |
| [`first-lora`](docs/recipes/first-lora.md) | 第一次 LoRA 微调会产出哪些 adapter、manifest、metrics | `make ai-lab`，然后看 [Training Runs](https://xianfeng92.github.io/finetune-lab/#/runs) |
| [`tool-calling`](docs/recipes/tool-calling.md) | SFT 样本如何教模型选择工具、填参数、输出 JSON | [Data Pipeline](https://xianfeng92.github.io/finetune-lab/#/data) + [Probe Compare](https://xianfeng92.github.io/finetune-lab/#/compare) |
| [`curriculum-vs-direct`](docs/recipes/curriculum-vs-direct.md) | 课程式训练和 direct mixed 训练在 probe 上有什么差异 | [Probe Compare](https://xianfeng92.github.io/finetune-lab/#/compare) + [real fine-tune guide](docs/ai/gemma4-real-finetune-guide.md) |

完整 recipe 索引见 [docs/recipes](docs/recipes/index.md)。每条 recipe 都给出标准命令、artifact 路径、Web 查看位置和防误读边界。
...
当前 W5/W6 证据很明确，但结论要写窄：MLX adapter 路径在 144 条 strict benchmark 上保留了 4 维 PolicyGateway 语义；同一 LoRA fuse 到 llama.cpp GGUF 后，`behavior_accuracy` 约 60%，`unsafe_direct_call_rate` 变成 24/144，`confirmation_contract_hit` 和 `refusal_contract_hit` 都掉到 0/12。fp16 与 Q4_K_M 几乎一致，说明主因不是量化，而是 Gemma 4 `num_kv_shared_layers` 在训练侧 compat shim 与 llama.cpp 推理侧之间的架构假设不一致。

这不是“三引擎 parity 排行榜”。LiteRT-LM 当前只跑通官方 base-only `.litertlm`，可用于说明无 tools 注入时的被动安全，但不能用于声称同一 LoRA 已在 LiteRT-LM 上保留 PolicyGateway 语义。
...
- [ ] Same-LoRA LiteRT-LM 路径验证后，再写 LiteRT runtime parity 结论
