...
> Fine-tuning is not about watching loss go down. It is about checking whether model behavior actually changed.

配图建议：

1. README / Web hero：`微调第一次看懂`
2. Loss Trap：loss 下降但 probe 失败的对照
3. Probe Diff：input / expected / actual case-level diff
4. Agent Handoff：`make ai-onboarding -> make ai-lab`
...
A visual lab for beginners to understand fine-tuning by watching model behavior change:
...
loss going down does not mean the model learned.
